    ...
    el.innerHTML = `
      <div style="background:var(--bg);border:1px solid var(--border);border-radius:8px;padding:18px 20px;margin-bottom:20px;">
        ...
        <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
          ...
          <button onclick="copyAddr('${refLink}',this)" style="padding:8px 16px;font-family:var(--font-mono);font-size:11px;letter-spacing:.08em;border:1px solid var(--border);border-radius:4px;background:var(--surface);color:var(--cream);cursor:pointer;white-space:nowrap;flex-shrink:0;" id="copyRefBtn">COPY</button>
        </div>
      </div>
      <div class="info-grid">
        <div class="info-cell">
        ...
        <div class="info-cell">
          ...
          <div class="info-cell-value">${user.registeredAt.toNumber() > 0 ? user.registeredAt.toNumber() : '—'}</div>
        </div>
      </div>
      ${referrals.length > 0 ? `
