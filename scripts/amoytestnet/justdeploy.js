...
// pre-deployed HDX token, seed the pool, warm up TWAP, then register the referral
// tree. This is inithdx.js up to PHASE 6 — it does NOT invest from any account.
//
// Referral tree (no investments):
...
//
// USAGE:
//   npx hardhat run scripts/amoytestnet/justdeploy.js --network polygonAmoy
...
const FUND_AMOUNT    = hre.ethers.parseEther("0.5");  // POL for gas (register)
...
// Accounts to register (acc[0] is the auto-registered deployer/owner).
const REGISTERED_ACCOUNTS = [1, 2, 3, 4, 5, 6, 7];
...
async function main() {
  ...
  console.log(sep("═"));
  console.log("  JUSTDEPLOY — Deploy · Seed · TWAP · Register (no invest)");
  console.log(sep("═"));
  ...
  // Deployer must cover: pool seed + 1 USDT registration fee for every account.
  const MIN_USDT = SEED_USDT
    + REGISTRATION_FEE * BigInt(REGISTERED_ACCOUNTS.length);
  if (usdtBal < MIN_USDT) {
  ...
  const configContent =
`// AUTO-GENERATED by scripts/amoytestnet/justdeploy.js
...
`;
  ...
  for (let i = 1; i < TOTAL_ACCOUNTS; i++) {
    ...
    // USDT: 1 registration fee only (no investment)
  ...
  console.log(`  Invested    : none (deploy + register only)`);
  console.log(sep("═") + "\n");
}
