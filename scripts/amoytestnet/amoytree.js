...
async function main() {
  ...
  console.log(sep("═"));
  console.log("  AMOYTREE — Deploy · Seed · TWAP · Chain register + invest");
  console.log(sep("═"));
  ...
  const recommendedPol = hre.ethers.parseEther("3") + FUND_AMOUNT * BigInt(CHAIN_LEN);
  if (deployerBal < recommendedPol) {
    console.warn(`⚠  recommended ≥ ${hre.ethers.formatEther(recommendedPol)} POL to fund all ${CHAIN_LEN} sub-wallets (continuing)`);
  }
  ...
  const configContent =
`// AUTO-GENERATED by scripts/amoytestnet/amoytree.js
...
`;
  ...
  // ── PHASE 5: Fund sub-wallets [1..15] (POL gas + USDT: reg fee + invest) ────
  console.log("\n" + sep()); console.log(`  PHASE 5 — FUND SUB-WALLETS [1..${CHAIN_LEN}]`); console.log(sep());
  for (let i = 1; i <= CHAIN_LEN; i++) {
    // POL for gas (registration + invest)
  ...
  // ── PHASE 6: Register the chain (acc[i] under acc[i-1]) ─────────────────────
  console.log("\n" + sep()); console.log("  PHASE 6 — REGISTER CHAIN"); console.log(sep());
